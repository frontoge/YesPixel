...
AddEventHandler('yp_mechanic:chargePlayer', function(target, amount)
	...
	xPlayer.addAccountMoney('bank', amount * 0.07)
	TriggerEvent('esx_addonaccount:getSharedAccount', 'society_mechanic', function(account)
		account.addMoney(amount * 0.93)
	end)
	TriggerClientEvent('mythic_notify:client:SendAlert', source, {type = 'success', text = 'You recieved a payment of $' .. amount * 0.07, length=4000})
	TriggerClientEvent('mythic_notify:client:SendAlert', target, {type = 'inform', text = 'You made a payment of $' .. amount, length=4000})
	exports['yp_taxes']:applyTax(source, 'income', amount * 0.07)
...
AddEventHandler('yp_mechanic:chargeForRepair', function(amount)
	...
	TriggerClientEvent('mythic_notify:client:SendAlert', source, {type = 'inform', text = 'You made a payment of $' .. amount .. ' for repairs', length=4000})
end)
