prints = 
{
	'pistolprint',
	'arprint',
	'sgprint',
	'smgprint',
	'meleeprint',
	'partsprint'
}

recipes = {
	{
		blueprint = 'pistolprint',
		label = 'Combat Pistol',
		value = 'WEAPON_COMBATPISTOL',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'pistolbarrel', count = 1},
					{item = 'pistolmag', count = 1}
				 }
	},
	{
		blueprint = 'pistolprint',
		label = 'Pistol',
		value = 'WEAPON_PISTOL',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'pistolbarrel', count = 1},
					{item = 'pistolmag', count = 1}
				 }
	},
	{
		blueprint = 'pistolprint',
		label = 'Desert Eagle',
		value = 'WEAPON_PISTOL50',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'pistolbarrel', count = 1},
					{item = 'armag', count = 1} --High Damage Weapon
				 }
	},
	{
		blueprint = 'pistolprint',
		label = 'SNS Pistol',
		value = 'WEAPON_SNSPISTOL',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'pistolmag', count = 1}
					--Small pistol, doesnt require barrel.
				 }
	},
	{
		blueprint = 'pistolprint',
		label = 'Heavy Pistol',
		value = 'WEAPON_HEAVYPISTOL',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'pistolbarrel', count = 1},
					{item = 'armag', count = 1}, --High Damage Weapon
					{item = 'plastic', count = 1}, --Gun Has Sight Attatchment
					{item = 'electronics', count = 1} --For Red Dot on sight
				 }
	},
	{
		blueprint = 'pistolprint',
		label = 'Vintage Pistol',
		value = 'WEAPON_VINTAGEPISTOL',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'pistolbarrel', count = 1},
					{item = 'pistolmag', count = 1}
				 }
	},
	{
		blueprint = 'pistolprint',
		label = 'Stun Gun',
		value = 'WEAPON_STUNGUN',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'pistolbarrel', count = 1},
					{item = 'pistolmag', count = 1},
					{item = 'electronics', count = 1}-- Needed for function
				 }
	},
	{
		blueprint = 'pistolprint',
		label = 'Revolver',
		value = 'WEAPON_REVOLVER',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'pistolbarrel', count = 1},
					{item = 'armag', count = 1}
				 }
	},
	{
		blueprint = 'pistolprint',
		label = 'Marksman Pistol',
		value = 'WEAPON_MARKSMANPISTOL',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'sggrip', count = 1},
					{item = 'pistolbarrel', count = 2},
					{item = 'pistolmag', count = 1}
				 }
	},
	{
		blueprint = 'pistolprint',
		label = 'Flare Gun',
		value = 'WEAPON_FLAREGUN',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'pistolbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'pistolbarrel', count = 1},
					{item = 'pistolmag', count = 1},
					{item = 'flint', count = 1}
				 }
	},
	{
		blueprint = 'smgprint',
		label = 'Micro SMG',
		value = 'WEAPON_MICROSMG',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'smgbody', count = 1},
					{item = 'smggrip', count = 1},
					{item = 'smgmag', count = 1}
				 }
	},
	{
		blueprint = 'smgprint',
		label = 'SMG',
		value = 'WEAPON_SMG',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'smgbody', count = 1},
					{item = 'smggrip', count = 1},
					{item = 'smgbarrel', count = 1},
					{item = 'smgmag', count = 1}
				 }
	},
	{
		blueprint = 'smgprint',
		label = 'Assault SMG',
		value = 'WEAPON_ASSAULTSMG',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'smgbody', count = 1},
					{item = 'smggrip', count = 1},
					{item = 'smgbarrel', count = 1},
					{item = 'smgmag', count = 1}
				 }
	},
	{
		blueprint = 'arprint',
		label = 'Combat PDW',
		value = 'WEAPON_COMBATPDW',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'arbody', count = 1},
					{item = 'argrip', count = 1},
					{item = 'arbarrel', count = 1},
					{item = 'armag', count = 1}
				 }
	},
	{
		blueprint = 'arprint',
		label = 'Gusenberg Sweeper',
		value = 'WEAPON_GUSENBERG',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'arbody', count = 1},
					{item = 'argrip', count = 1},
					{item = 'arbarrel', count = 1},
					{item = 'ardrum', count = 1},
					{item = 'arstock', count  = 1}
				 }
	},
	{
		blueprint = 'arprint',
		label = 'Assault Rifle',
		value = 'WEAPON_ASSAULTRIFLE',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'arbody', count = 1},
					{item = 'argrip', count = 1},
					{item = 'arbarrel', count = 1},
					{item = 'armag', count = 1},
					{item = 'arstock', count = 1},
					--{item = 'wood', count = 2} --Wood for Wooden Stock
				 }
	},
	{
		blueprint = 'arprint',
		label = 'Carbine Rifle',
		value = 'WEAPON_CARBINERIFLE',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'arbody', count = 1},
					{item = 'argrip', count = 1},
					{item = 'arbarrel', count = 1},
					{item = 'armag', count = 1},
					{item = 'arstock', count = 1},
					{item = 'metal', count = 2}
				 }
	},
	{
		blueprint = 'sgprint',
		label = 'Pump Shotgun',
		value = 'WEAPON_PUMPSHOTGUN',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'sgbody', count = 1},
					{item = 'sggrip', count = 1},
					{item = 'sgbarrel', count = 2},
					{item = 'sgstock', count = 1}
				 }
	},
	{
		blueprint = 'sgprint',
		label = 'Sawed-Off Shotgun',
		value = 'WEAPON_SAWNOFFSHOTGUN',
		recipe = {
					{item = 'spring', count = 1},
					{item = 'sgbody', count = 1},
					{item = 'pistolgrip', count = 1},
					{item = 'sgbarrel', count = 1},
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Knife',
		value = 'WEAPON_KNIFE',
		recipe = {
					{item = 'metal', count = 1},
					{item = 'plastic', count = 1}
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Brass Knuckles',
		value = 'WEAPON_KNUCKLE',
		recipe = {
					{item = 'metal', count = 5}	
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Nightstick',
		value = 'WEAPON_NIGHTSTICK',
		recipe = {
					{item = 'wood', count = 3}
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Hammer',
		value = 'WEAPON_HAMMER',
		recipe = {
					{item = 'metal', count = 2},
					{item = 'wood', count = 1}
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Bat',
		value = 'WEAPON_BAT',
		recipe = {
					{item = 'metal', count = 5}
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Golf Club',
		value = 'WEAPON_GOLFCLUB',
		recipe = {
					{item = 'metal', count = 3},
					{item = 'rubber', count = 1}
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Broken Bottle',
		value = 'WEAPON_BOTTLE',
		recipe = {
					{item = 'glass', count = 3}
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Machete',
		value = 'WEAPON_MACHETE',
		recipe = {
					{item = 'metal', count = 2},
					{item = 'wood', count = 1}
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Switchblade',
		value = 'WEAPON_SWITCHBLADE',
		recipe = {
					{item = 'metal', count = 2},
					{item = 'plastic', count = 1},
					{item = 'spring', count = 1}
				 }
	},
	{
		blueprint = 'meleeprint',
		label = 'Pool Cue',
		value = 'WEAPON_POOLCUE',
		recipe = {
					{item = 'wood', count = 3}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'AR Body',
		value = 'arbody',
		recipe = {
					{item = 'metal', count = 10}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'AR Magazine',
		value = 'armag',
		recipe = {
					{item = 'plastic', count = 3},
					{item = 'spring', count = 2},
					{item = 'metal', count = 3}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'AR Grip',
		value = 'argrip',
		recipe = {
					{item = 'metal', count = 6},
					{item = 'rubber', count = 5}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'AR Barrel',
		value = 'arbarrel',
		recipe = {
					{item = 'metal', count = 10}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'AR Stock',
		value = 'arstock',
		recipe = {
					{item = 'metal', count = 6}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'AR Drum',
		value = 'ardrum',
		recipe = {
					{item = 'metal', count = 10}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'Pistol Body',
		value = 'pistolbody',
		recipe = {
					{item = 'metal', count = 3}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'Pistol Magazine',
		value = 'pistolmag',
		recipe = {
					{item = 'plastic', count = 1},
					{item = 'spring', count = 2},
					{item = 'metal', count = 2}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'Pistol Grip',
		value = 'pistolgrip',
		recipe = {
					{item = 'metal', count = 2},
					{item = 'rubber', count  = 3}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'Pistol Barrel',
		value = 'pistolbarrel',
		recipe = {
					{item = 'metal', count = 4}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'SMG Body',
		value = 'smgbody',
		recipe = {
					{item = 'metal', count = 6}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'SMG Magazine',
		value = 'smgmag',
		recipe = {
					{item = 'plastic', count = 4},
					{item = 'spring', count = 4},
					{item = 'metal', count = 3}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'SMG Grip',
		value = 'smggrip',
		recipe = {
					{item = 'metal', count = 2},
					{item = 'rubber', count  = 4}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'SMG Barrel',
		value = 'smgbarrel',
		recipe = {
					{item = 'metal', count = 6}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'SMG Stock',
		value = 'smgstock',
		recipe = {
					{item = 'metal', count = 5}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'Shotgun Body',
		value = 'sgbody',
		recipe = {
					{item = 'metal', count = 6}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'Shotgun Barrel',
		value = 'sgbarrel',
		recipe = {
					{item = 'metal', count = 8} --Shotgun barrels need more metal than other guns
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'Shotgun Grip',
		value = 'sggrip',
		recipe = {
					{item = 'metal', count = 5},
					{item = 'rubber', count  = 4}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'Shotgun Stock',
		value = 'sgstock',
		recipe = {
					{item = 'metal', count = 4}
				 }
	},
	{
		blueprint = 'partsprint',
		label = 'Spring',
		value = 'spring',
		recipe = {
					{item = 'metal', count = 2}
				 }
	}
}	
